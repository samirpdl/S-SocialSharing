...
//  SInstagram.m
//
//  Created by Samir Poudel on 08/14/14.
...
{
    static SInstagram* sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[SInstagram alloc] init];
    });
...
+ (void) setPhotoFileName:(NSString*)fileName {
    [SInstagram sharedInstance].photoFileName = fileName;
}
+ (NSString*) photoFileName {
    return [SInstagram sharedInstance].photoFileName;
}
...
+ (void) postImage:(UIImage*)image withCaption:(NSString*)caption inView:(UIView*)view delegate:(id<UIDocumentInteractionControllerDelegate>)delegate {
    [[SInstagram sharedInstance] postImage:image withCaption:caption inView:view delegate:delegate];
}
